...
set.seed(1111)
sim_panel_1111 <- sim_panel(nx = 3,
                            nfacet = 2,
                            ntimes = 4,
                            sim_dist = sim_varx_normal(3, 2, 0, 1, w = 3)) %>%
  unnest(data)
set.seed(9999)
sim_panel_9999 <- sim_panel(nx = 3,
                            nfacet = 2,
                            ntimes = 4,
                            sim_dist = sim_varx_normal(3, 2, 0, 1, w = 3)) %>%
  unnest(data)
...
test_that("different seeds produce different observations", {
  ...
  expect_equal(sim_panel_1111$sim_data - sim_panel_9999$sim_data,
               c(-1.17067923, 0.47941557, 0.14531299, 1.94780263, -2.78752574,
...
set.seed(1111)
simf_1111 <- sim_panel(nx = 3,
                            nfacet = 2,
                            ntimes = 4,
                            sim_dist = sim_varf_normal(3, 2, 0, 1, w = 3)) %>%
  unnest(data)
set.seed(9999)
simf_9999 <- sim_panel(nx = 3,
                            nfacet = 2,
                            ntimes = 4,
                            sim_dist = sim_varf_normal(3, 2, 0, 1, w = 3)) %>%
  unnest(data)
...
test_that("different seeds produce different observations", {
  ...
  expect_equal(simf_1111$sim_data - simf_9999$sim_data,
               c(-1.17067923, 0.47941557, 0.14531299, 1.94780263, -2.78752574,
                 -3.83973030,  1.64058471,  1.30606069, -0.03632688,  0.29750263,
                 -0.52653933, -0.08082995, -1.11771041, -1.38786226,  0.47229198,
                 1.16244362,  0.09872577, -3.29538290,  0.41125692, -1.99185321,
...
expect_equal(-simf_1111$sim_data + sim_panel_1111$sim_data,
...
set.seed(1111)
simv_1111 <- sim_panel(nx = 3,
                            nfacet = 2,
                            ntimes = 4,
                            sim_dist = sim_varall_normal(3, 2, 0, 1, w = 3)) %>%
  unnest(data)
set.seed(9999)
simv_9999 <- sim_panel(nx = 3,
                            nfacet = 2,
                            ntimes = 4,
                            sim_dist = sim_varall_normal(3, 2, 0, 1, w = 3)) %>%
  unnest(data)
